<template>
    <div class="text-white flex flex-col focus:shadow-lg focus:shadow-blue-800 justify-between pl-12 w-full py-4 bg-gray-700 rounded-xl outline-none hover:shadow-blue-800 shadow-lg transition-all duration-300 ease-in" @keyup.escape = "close()">
        <div v-for="datos in datos" class="m-1 flex flex-row bg-gray-700 border-b border-solid border-gray-600 max-w-90 min-w-min justify-between">
            <div>
                <div class="max-w-80 min-w-min flex overflow-x-hidden">
                    <div class="mr-1">{{ datos.texto }}</div>
                </div>
            </div>
